...
```sh
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/nonathaj/agent-meter/releases/latest/download/agent-meter-installer.sh | sh
```

On Windows:

```powershell
powershell -ExecutionPolicy Bypass -c "irm https://github.com/nonathaj/agent-meter/releases/latest/download/agent-meter-installer.ps1 | iex"
```
...
Bump `version` in `Cargo.toml`, commit, and push a matching tag
(`git tag v0.2.0 && git push --tags`). The release workflow, generated by
[dist](https://opensource.axo.dev/cargo-dist/) from `dist-workspace.toml`,
builds every platform and publishes the release with its installers. After
changing `dist-workspace.toml`, run `dist generate` rather than editing
...
there — on the `PATH`, in `~/.cargo/bin` or `~/.local/bin`, or wherever
`--command` says — and ssh must reach it without asking you anything. The
first two are looked in because a command run over ssh does not read
`~/.profile`, so on most Linux machines neither is on the `PATH` it gets.
